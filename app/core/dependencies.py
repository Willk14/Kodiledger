...
# ============================================================
# Normal application repositories
# ============================================================
...
# ============================================================
# System repositories - webhook/integration path
# ============================================================

def get_system_landlord_repository(
    db: AsyncSession = Depends(get_system_db),
) -> LandlordRepository:
    return LandlordRepository(db)


def get_system_tenant_repository(
    db: AsyncSession = Depends(get_system_db),
) -> TenantRepository:
    return TenantRepository(db)


def get_system_webhook_repository(
    db: AsyncSession = Depends(get_system_db),
) -> WebhookRepository:
    return WebhookRepository(db)


def get_system_payment_processing_repository(
    db: AsyncSession = Depends(get_system_db),
) -> PaymentProcessingRepository:
    return PaymentProcessingRepository(db)


def get_system_payment_transaction_repository(
    db: AsyncSession = Depends(get_system_db),
) -> PaymentTransactionRepository:
    return PaymentTransactionRepository(db)


def get_system_payment_allocation_repository(
    db: AsyncSession = Depends(get_system_db),
) -> PaymentAllocationRepository:
    return PaymentAllocationRepository(db)


def get_system_invoice_repository(
    db: AsyncSession = Depends(get_system_db),
) -> InvoiceRepository:
    return InvoiceRepository(db)


def get_system_payment_credit_repository(
    db: AsyncSession = Depends(get_system_db),
) -> PaymentCreditRepository:
    return PaymentCreditRepository(db)


def get_system_ledger_repository(
    db: AsyncSession = Depends(get_system_db),
) -> LedgerRepository:
    return LedgerRepository(db)


def get_system_unassigned_payment_repository(
    db: AsyncSession = Depends(get_system_db),
) -> UnassignedPaymentRepository:
    return UnassignedPaymentRepository(db)


def get_system_outbox_event_repository(
    db: AsyncSession = Depends(get_system_db),
...
def get_outbox_event_service(
    repository: OutboxEventRepository = Depends(
        get_system_outbox_event_repository
    ),
) -> OutboxEventService:
...
def get_invoice_allocation_service(
    invoice_repository: InvoiceRepository = Depends(
        get_system_invoice_repository
    ),
    payment_allocation_repository: PaymentAllocationRepository = Depends(
        get_system_payment_allocation_repository
    ),
...
def get_reconciliation_service(
    ...
    ledger_repository: LedgerRepository = Depends(
        get_system_ledger_repository
    ),
    unassigned_payment_repository: UnassignedPaymentRepository = Depends(
        get_system_unassigned_payment_repository
    ),

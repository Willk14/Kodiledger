...
async def test_overpayment_allocates_invoice_and_creates_payment_credit():
    ...
    try:
        ...
        async with TestSystemSessionLocal() as db:
            ...
            allocation_row = allocation.mappings().first()

            assert allocation_row is not None
            assert allocation_row["amount"] == invoice_amount
            assert str(allocation_row["status"]) == "ALLOCATED"
            assert str(allocation_row["invoice_id"]) == invoice_id
            ...
            credit_row = credit.mappings().first()

            assert credit_row is not None
            assert credit_row["amount"] == expected_credit
            assert str(credit_row["status"]) == "AVAILABLE"
            assert str(credit_row["tenant_id"]) == TENANT_ID

            assert str(credit_row["payment_transaction_id"]) == transaction_id
